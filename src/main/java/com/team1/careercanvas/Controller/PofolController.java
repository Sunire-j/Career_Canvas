...
public class PofolController {
    ...
    public ModelAndView pofolWriteTeam(HttpSession session) {
        ModelAndView mav = new ModelAndView();
        List<PartyVO> partylist = partymapper.getMineParty((String) session.getAttribute("LogId"));
        List<PartyVO> memberList = mapper.getPartyList((String) session.getAttribute("LogId"));
